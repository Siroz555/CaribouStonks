...
public record SkyBlockTime(
	...
	private static final double HOUR_MILLIS = 50_000;
	private static final double DAY_MILLIS = HOUR_MILLIS * 24;
	private static final double MONTH_MILLIS = DAY_MILLIS * 31;
	private static final double SEASON_MILLIS = MONTH_MILLIS * 3;
	private static final double YEAR_MILLIS = SEASON_MILLIS * 4;

	public static SkyBlockTime of(long millis) {
		return new SkyBlockTime(
				(int) (Math.floor(millis / YEAR_MILLIS) + 1), // 1-based
				(int) (Math.floor(millis / MONTH_MILLIS) % 12), // 0-based
				(int) (Math.floor(millis / DAY_MILLIS) % 31 + 1), // 1-based
				(int) (Math.floor(millis / HOUR_MILLIS) % 24)  // 0-based
		);
	}
