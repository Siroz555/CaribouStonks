...
final class PetValueCalculator {
	...
	public static ItemValueResult handle(PetInfo petInfo, ToDoubleFunction<String> prices, boolean networth) {
		...
		if (lvl1 == 0 || (lvl100 == 0 && !CalculatorConstants.PET_SPECIALS.containsKey(petInfo.type()))) {
	...
	private static int calculatePetLevel(PetInfo info) {
		String petRarity = info.heldItem().orElse("").equals(TIER_BOOST_ITEM_ID)
				? PET_RARITIES.get(PET_RARITIES.indexOf(info.tier()) + 1)
				: info.tier();
		int maxPetLevel = CalculatorConstants.PET_SPECIALS.getOrDefault(info.type(), 100);
	...
	private static double worth(String skyBlockId, boolean networth) {
