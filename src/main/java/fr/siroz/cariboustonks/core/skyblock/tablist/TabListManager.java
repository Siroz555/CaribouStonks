...
public final class TabListManager {
	...
	private static final Comparator<PlayerListEntry> TAB_ORDER = Comparator
			.<PlayerListEntry, Integer>comparing(p -> p.getGameMode() == GameMode.SPECTATOR ? 1 : 0)
			.thenComparing(p -> p.getScoreboardTeam() != null ? p.getScoreboardTeam().getName() : "")
	...
	public TabListManager() {
		ClientPlayConnectionEvents.JOIN.register((_h, _s, _c) -> this.lastWorldChange = System.currentTimeMillis());
	 ...
	 */
	public Optional<TabWidget> get(String widgetName) {
	 ...
	 */
	public List<TabWidget> getWidgetView() {
	 ...
	 */
	public Optional<String> findLine(String widgetName, String keyword) {
	...
	private void updateTabList() {
		try {
			...
			HudEvents.TAB_LIST_UPDATE.invoker().onUpdate(stringTab);
		} catch (Exception ignored) {
		}
	}

	private List<TabWidget> extractWidgets(List<TabLine> lines) {
