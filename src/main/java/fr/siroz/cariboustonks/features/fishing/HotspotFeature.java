...
public class HotspotFeature extends Feature {
	...
	private void onParticleReceived(ClientboundLevelParticlesPacket particle) {
		...
		Vec3 particlePos = new Vec3(particle.getX(), particle.getY(), particle.getZ());
	...
	private void onHotspotFound(Hotspot hotspot) {
		...
		TextColor textColor = MinecraftUtils.findStyle(hotspot.perk).getColor();
		Color color = textColor == null ? Color.fromFormatting(ChatFormatting.WHITE) : Color.fromInt(textColor.getValue());
