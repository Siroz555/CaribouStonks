...
public class LotusAtollBuffFeature extends Feature {
	...
	private void notifyExpired() {
		...
		if (this.config().fishing.lotusAtoll.buffExpiredWarn) {
			...
			Client.showSubtitle(Component.literal("Buffs!").withStyle(ChatFormatting.DARK_AQUA, ChatFormatting.BOLD), 1, 30, 1);
			Client.playSound(SoundEvents.ALLAY_HURT, 1f, 1f);
		}
	}
