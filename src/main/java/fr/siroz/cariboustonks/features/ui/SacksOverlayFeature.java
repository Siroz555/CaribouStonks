...
public class SacksOverlayFeature extends Feature {
	...
	private void render(GuiGraphics guiGraphics, int screenWidth, int screenHeight, int x, int y) {
		...
		for (Line line : lines) {
			...
			colWidth[0] = Math.max(colWidth[0], CLIENT.font.width(line.name()));
			if (line.storedAndAmount() != null) {
				colWidth[1] = Math.max(colWidth[1], CLIENT.font.width(line.storedAndAmount()));
			}
			if (line.value() != null) {
				colWidth[2] = Math.max(colWidth[2], CLIENT.font.width(line.value()));
			}
		}
		...
		for (Line line : lines) {
			...
			if (hasIcon) {
				guiGraphics.renderItem(line.item(), baseX, baseY + offset);
				int textX = baseX + ICON_SIZE + ICON_GAP;
				int textY = baseY + offset + 4; // centrage vertical

				guiGraphics.drawString(CLIENT.font, line.name(), textX, textY, Colors.WHITE.asInt());
				textX += colWidth[0] + cellPadding;

				if (line.storedAndAmount() != null) {
					guiGraphics.drawString(CLIENT.font, line.storedAndAmount(), textX, textY, Colors.WHITE.asInt());
					textX += colWidth[1] + cellPadding;
				}

				if (line.value() != null) {
					guiGraphics.drawString(CLIENT.font, line.value(), textX, textY, Colors.WHITE.asInt());
				}

				offset += ICON_SIZE;
			} else {
				guiGraphics.drawString(CLIENT.font, line.name(), baseX, baseY + offset, Colors.WHITE.asInt());
				offset += lineHeight;
			}
		}
	...
	private @NonNull Component truncateName(@NonNull Component name) {
		if (CLIENT.font.width(name) <= MAX_NAME_WIDTH) return name;

		String text = name.getString();
		Style style = findStyle(name);

		// Réduit caractère par caractère jusqu'à rentrer avec "..."
		while (!text.isEmpty() && CLIENT.font.width(text + "…") > MAX_NAME_WIDTH) {
