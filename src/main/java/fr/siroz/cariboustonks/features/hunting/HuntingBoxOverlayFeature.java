...
public class HuntingBoxOverlayFeature extends Feature {
	...
	public HuntingBoxOverlayFeature() {
		...
		ScreenEvents.AFTER_INIT.register((_mc, screen, _a, _b) -> {
			ScreenEvents.afterRender(screen).register(this::render);
			ScreenEvents.remove(screen).register(_s -> this.lines.clear());
		});
	...
	private void render(Screen screen, GuiGraphics guiGraphics, int mouseX, int mouseY, float tickDelta) {
		...
		for (Line line : lines) {
			...
			if (line.itemStack() != null) {
				guiGraphics.renderItem(line.itemStack(), PADDING_LEFT, y);
				leftToRightPadding = true;
			}
			int xPadding = leftToRightPadding ? LEFT_TO_RIGHT : 0;
			guiGraphics.drawString(CLIENT.font, line.text(), PADDING_LEFT + xPadding, y + 2, Colors.WHITE.asInt());
			y += LINE_HEIGHT;
		}
