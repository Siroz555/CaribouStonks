...
public class StonksFeature extends Feature {
	...
	public void stonksItem(@NonNull ItemStack stack) {
		...
		if (neuId.isEmpty() || hypixelSkyBlockId.isEmpty()) {
			...
			PlayerContext.sendErrorMessage("Unable to identify item " + stack.getHoverName().getString(), false);
			if (DeveloperTools.isInDevelopment()) {
				String extra = "(id: " + neuIdError + " | skyBlockApiId: " + hypixelSkyBlockIdError + ")";
				PlayerContext.sendMessage(Component.literal(extra).withStyle(ChatFormatting.DARK_GRAY));
			}
			...
			ClientContext.setScreen(StonksScreen.create(ItemLookupKey.of(neuId, hypixelSkyBlockId)));
		}
	}
