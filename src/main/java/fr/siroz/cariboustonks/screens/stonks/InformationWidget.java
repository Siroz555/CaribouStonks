...
class InformationWidget extends AbstractStonksWidget {
	...
	private void renderBazaar(@NonNull BazaarProduct bazaarItem, @NonNull GuiGraphicsExtractor guiGraphics, int x, int y) {
		// Buy
		...
		guiGraphics.text(textRenderer, textVelocity, x + 20, y + 160, Colors.WHITE.asInt());
		...
		if (ConfigManager.getConfig().general.stonks.showAllDataInInfoScreen) {
			...
			guiGraphics.text(textRenderer, warningInfo, x + 20, y + 225, Colors.WHITE.asInt());
		}
	}

	private void renderAuction(@NonNull AuctionStatistics stats, @NonNull GuiGraphicsExtractor guiGraphics, int x, int y) {
		// ---------- Aujourd'hui ----------
		...
		if (today.daysWithData() > 0) {
			...
			guiGraphics.text(textRenderer, soldTodayNoData, x + 20, y + 20, Colors.WHITE.asInt());
		}
		...
		guiGraphics.text(textRenderer, sold7d, x + 20, y + 60, Colors.WHITE.asInt());
		...
		if (week.hasPriceData()) {
			...
			guiGraphics.text(textRenderer, warningWeekPrice, x + 20, y + 70, Colors.WHITE.asInt());
		}
		...
		guiGraphics.text(textRenderer, sold30d, x + 20, y + 90, Colors.WHITE.asInt());
		...
		if (month.hasPriceData()) {
			...
			guiGraphics.text(textRenderer, warningMouthPrice, x + 20, y + 100, Colors.WHITE.asInt());
		}
		...
		guiGraphics.text(textRenderer, activeDays, x + 20, y + 120, Colors.WHITE.asInt());
		...
		if (stats.last7Days().daysWithData() != 0 && stats.last30Days().daysWithData() != 0) {
			...
			guiGraphics.text(textRenderer, textTrend, x + 20, y + 130, Colors.WHITE.asInt());

			Component textTrendInfo = Component.literal("(Week vs. mouth comparison)").withStyle(ChatFormatting.DARK_GRAY);
			guiGraphics.text(textRenderer, textTrendInfo, x + 20, y + 140, Colors.WHITE.asInt());
		} else {
			Component warningTrend = Component.literal("Trend Error: No Data").withStyle(ChatFormatting.RED);
			guiGraphics.text(textRenderer, warningTrend, x + 20, y + 130, Colors.WHITE.asInt());
		}
		...
		if (week.hasPriceData() && month.hasPriceData() && month.avgPrice() > 0) {
			...
			guiGraphics.text(textRenderer, textTrend, x + 20, y + 155, Colors.WHITE.asInt());

			Component textPriceTrendInfo = Component.literal("(Price trend over 7d to 30 days)").withStyle(ChatFormatting.DARK_GRAY);
			guiGraphics.text(textRenderer, textPriceTrendInfo, x + 20, y + 165, Colors.WHITE.asInt());
		} else {
			Component warningPriceTrend = Component.literal("Price Trend Error: No Data").withStyle(ChatFormatting.RED);
			guiGraphics.text(textRenderer, warningPriceTrend, x + 20, y + 155, Colors.WHITE.asInt());
		}
		...
		guiGraphics.text(textRenderer, warningInfo, x + 20, y + 185, Colors.WHITE.asInt());
	}
}
