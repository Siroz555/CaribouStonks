...
public class ReminderScreen extends CaribousStonksScreen {
    ...
    protected void onInit() {
		...
		createButton = adder.addChild(Button.builder(Component.literal("Create custom Reminder"), _ -> {

		}).tooltip(Tooltip.create(Component.literal("3-5 Business days"))).build());

		optionButton = adder.addChild(Button.builder(Component.literal("Options"), _ -> {

		}).tooltip(Tooltip.create(Component.literal("3-5 Business days"))).build());

		adder.addChild(Button.builder(CommonComponents.GUI_DONE, _ -> close()).width(310).build(), 2);
